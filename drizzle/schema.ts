...
/**
 * House View (Agentic Ripple, Phase D). The single daily "alpha" card — the
 * house's opinionated cross-cutting read on the current signals, for the
 * Singapore-professional persona. One row per brief date (upserted); generated by
 * a single Sonnet call chained off publish, like theme synthesis. Read-time never
 * calls an LLM.
 */
export interface HouseViewRef {
  slug: string;
  storyIndex: number;
  text: string;
}
export const houseView = pgTable("house_view", {
  id: serial("id").primaryKey(),
  /** ISO brief date this view was generated for, "YYYY-MM-DD" */
  date: text("date").notNull().unique(),
  headline: text("headline").notNull(),
  thesis: text("thesis").notNull(),
  /** Short conviction line, e.g. "Constructive on the energy risk premium" */
  stance: text("stance").notNull().default(""),
  /** The open signals the view leans on, for a "reasoning trail" */
  signalRefs: jsonb("signal_refs").notNull().$type<HouseViewRef[]>().default([]),
  model: text("model").notNull().default(""),
  generatedAt: bigint("generated_at", { mode: "number" }).notNull().$defaultFn(() => Date.now()),
});
export type HouseView = typeof houseView.$inferSelect;
export type InsertHouseView = typeof houseView.$inferInsert;
