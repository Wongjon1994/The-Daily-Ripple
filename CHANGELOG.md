...
- Moved early specs, n8n pipeline guides, and agent handoff notes into `docs/internal/`,
  and deleted a dead one-off-script cluster (`insert-may31-brief.*`, `may31_brief_data.json`,
  `n8n-workflow-export.json`, `template.json`). Root now holds only the README, CHANGELOG,
  and current-behaviour specs. `CLAUDE.md` stays at root (Claude Code loads it from there)
  and gained a "keep docs in sync" standing rule.
- Synced the two `briefParser` copies (client/server) byte-identical — the drift was
  cosmetic (identical output), not the latent bug it looked like. No app behaviour change.
...
- **Grounding guardrail on the House View** — the daily alpha synthesis was stating
  ...
  when SORA was ~1%). The generation prompt (`server/houseView.ts`) now forbids any
  price/rate/yield/index **level that isn't written verbatim in the source signals** —
  it reasons in directions and triggers ("reprices SORA upward") instead of fabricated
  thresholds. Keeps the view specific about mechanisms without ungrounded numbers.
